		...
		for i in range (0, rows):
			for j in range(0, columns):
				if (original_image[i, j] == 0):
					...
					for a in range(-1,2):
						for b in range(-1,2):
							if (IsIndexValid(i+a, j+b, rows, columns) and original_image[i+a, j+b] == 255):
								flag = True
								break
					if (flag == True): 
						result[i,j] = 255
...
#We detect that it s a double line road, and rw = 34 and lw = 1
rw = 34
d = Dilation(image, 2)
cv2.imwrite("Map 3 Dilated.png", d)
e = Erosion(d, 4)
cv2.imwrite("Map 3 Eroded.png", e)
t = Thinning(e)
cv2.imwrite("Map 3 Thinned.png", t)
...
#cv2.imwrite("TjunctionThinningWIthoutErosion.png", t)
#print(bwimage)
